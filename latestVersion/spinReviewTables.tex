...
This distinction is done only in the this (first) section and App.~\ref{sec:vocabulary}.
...
\end{equation}
with $\hbar \omega_\mathrm{q}$ the energy difference of the qubit excited and ground states, $g$ the qubit--circuit coupling, $D$ the difference of the population probability of the qubit ground and excited state, and $\Gamma_2 = \Gamma_1/2 +\Gamma_\varphi$ is defined in App.~\ref{app:notation}. These formulas are also valid when the qubit itself is driven, the so-called two-tone spectroscopy\cite{scarlino_all-microwave_2019}. In that case, all parameters on the right hand side of Eq.~\eqref{eq:susceptibility} should be replaced by the corresponding quantities in the rotating reference frame \cite{ithier_decoherence_2005}.
